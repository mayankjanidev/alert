<div role="alert" {{ $attributes->class(['flex items-center justify-between p-3 rounded-md']) }}>
    <div class="flex items-center">
        ...
        @endif
        <div>
            <div @class([ 'font-medium' , 'text-lg'=> $description])>{{ $title }}</div>
            @if($description)
            <div>{{ $description }}</div>
            @endif
        </div>
    ...
    <div class="cursor-pointer" onclick="parentNode.remove()" aria-label="Close">
        <svg @class([ 'w-8 h-8 fill-current'=> $description,
            'w-5 h-5 fill-current' => !$description]) xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
